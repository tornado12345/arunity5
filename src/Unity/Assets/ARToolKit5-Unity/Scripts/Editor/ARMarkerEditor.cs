...
//    	{ARController.ARToolKitMatrixCodeType.AR_MATRIX_CODE_GLOBAL_ID, 18446744073709551616}
    ...
    private void DrawPatterns(ARMarker m)
    {
        const float SingleLineHeight = 9f;
        const float PatternHeight = 40f + SingleLineHeight;

        scrollPosition = EditorGUILayout.BeginScrollView(
            scrollPosition,
            GUILayout.Height(
                PatternHeight + GUI.skin.horizontalScrollbar.fixedHeight));
                ...
                EditorGUILayout.LabelField(
                    new GUIContent(
                        string.Format(
                            "Pattern {0}, {1} m",
                            i,
                            m.Patterns[i].width.ToString("n3")),
                        m.Patterns[i].texture),
                    GUILayout.Height(PatternHeight)); // n3 -> 3 decimal places.
            }
        }
