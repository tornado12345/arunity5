...
public static class PluginFunctions
{
	...
	public static int arwGetTrackedMarkerPosition(int index, float[] vertex, out int markerID)
	{
		if (Application.platform == RuntimePlatform.IPhonePlayer) return ARNativePluginStatic.arwGetTrackedMarkerPosition(index, vertex, out markerID);
		else return ARNativePlugin.arwGetTrackedMarkerPosition(index, vertex, out markerID);
	}

	public static bool arwGetIdeal2Observed(float ix, float iy, out float ox, out float oy)
	{
		if (Application.platform == RuntimePlatform.IPhonePlayer)
			return ARNativePluginStatic.arwGetIdeal2Observed (ix, iy, out ox, out oy);
		else
			return ARNativePlugin.arwGetIdeal2Observed (ix, iy, out ox, out oy);
	}
